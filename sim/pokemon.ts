...
export class Pokemon {
	...
	*/
	...
	runImmunity(type: TypeName, message?: string | boolean) {
	...
	runStatusImmunity(effectid: string, message?: string) {
		if (this.fainted) return false;
		if (!effectid) return true;

		if (!this.battle.dex.getStatusImmunity(effectid, this)) {
		...
		const immunity = this.battle.runEvent('Immunity', this, null, null, effectid);
		if (!immunity) {
